...
# Some custom functions to process the data
# -----------------------------------------
...
#         print(file.split('/')[-1].split("KGWX")[1].split("_V")[0])
        file_datetime_str = file.split('/')[-1].split(radar_name)[1].split("_V")[0]
...
with multiprocessing.Pool() as pool:
    pool.map(partial_process_file, data_files)


    
# if multiprocessing fails, try serially,
# just remove ''' ''' quote signs below.
# ======================================
