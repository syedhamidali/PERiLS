...
savedir = f"{basedir}{'plots/uf'}{os.sep}"
...
for cfile in uf_files:
    ...
    filenames = os.path.join(savedir, cfile.split(os.sep)[-1].split('_to_')[-1].split('.nc')[0] + '_uf.png')
...
images[0].save(os.path.join(savedir,'uf_iop1_cow1_animation.gif'), save_all=True, append_images=images[1:], duration=50*1000/len(images), loop=0)
